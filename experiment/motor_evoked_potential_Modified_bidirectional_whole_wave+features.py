...
# Initialise targets with all samples as backgroud noise 
targets=np.ones((num_samples,4));
targets[:,1:]=0;
...
for i in range(num_sine_waves-1):
    ...
    # 
    targets[start:end,:]=np.array([0,1,0,0]);# whole wave
    targets[crest_pos,:]=np.array([0,1,1,0]);# 90deg
    targets[trough_pos,:]=np.array([0,1,0,1]);#270deg
